...
// Per-LED color state for individual LED control
static struct zmk_led_hsb pixel_colors[STRIP_NUM_PIXELS];
static bool pixel_on[STRIP_NUM_PIXELS];
...
int zmk_rgb_underglow_on_idx(int index) {
    if (!led_strip || index < 0 || index >= STRIP_NUM_PIXELS)
        return -ENODEV;

    pixel_on[index] = true;
    pixel_colors[index] = state.color;

    pixels[index] = hsb_to_rgb(hsb_scale_min_max(pixel_colors[index]));
    int err = led_strip_update_rgb(led_strip, pixels, STRIP_NUM_PIXELS);
    if (err < 0) {
        LOG_ERR("Failed to update LED %d (%d)", index, err);
        return err;
    }
...
int zmk_rgb_underglow_off_idx(int index) {
    if (!led_strip || index < 0 || index >= STRIP_NUM_PIXELS)
        return -ENODEV;

    pixel_on[index] = false;
    pixels[index] = (struct led_rgb){r : 0, g : 0, b : 0};
    int err = led_strip_update_rgb(led_strip, pixels, STRIP_NUM_PIXELS);
    if (err < 0) {
        LOG_ERR("Failed to update LED %d (%d)", index, err);
        return err;
...
int zmk_rgb_underglow_toggle_idx(int index) {
    if (!led_strip || index < 0 || index >= STRIP_NUM_PIXELS)
        return -ENODEV;

    return pixel_on[index] ? zmk_rgb_underglow_off_idx(index) : zmk_rgb_underglow_on_idx(index);
}
